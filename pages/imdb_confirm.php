...
<input type="hidden" name="imdb" value="<?=$obj->getIMDB()?>"/>
<input type="hidden" name="image" value="<?=$obj->getImage()?>"/>
...
<tr>
	...
	<tr>
		<td rowspan="6" width="105"><img src="<? if (is_null($obj->getImage())) { print 'images/noimage.gif'; } else { print TEMP_FOLDER.$obj->getImage(); }?>" border="0" class="imgx"/></td>
		<td width="30%"><?=$language->show('M_TITLE')?>:</td>
		<td width="50%"><input type="text" name="title" value="<?=VCDUtils::titleFormat($obj->getTitle())?>" size="28"></td>
	</tr>
	<tr>
		<td>IMDB <?=$language->show('M_TITLE')?>:</td>
		<td><input type="text" name="imdbtitle" value="<?=$obj->getTitle()?>" size="28"></td>
	</tr>
	<tr>
		<td><?=$language->show('M_ALTTITLE')?>:</td>
		<td><input type="text" name="alttitle" value="<?=$obj->getAltTitle()?>" size="28"></td>
	</tr>
	<tr>
		<td><?=$language->show('M_YEAR')?>:</td>
		<td><input type="text" name="year" value="<?=$obj->getYear()?>" size="4"></td>
	</tr>
	<tr>
		<td><?=$language->show('M_GRADE')?>:</td>
		<td><input type="text" name="rating" value="<?=$obj->getRating()?>" size="2"> stars</td>
	</tr>
	<tr>
		<td><?=$language->show('M_RUNTIME')?>:</td>
		<td><input type="text" name="Runtime" value="<?=$obj->getRuntime()?>" size="4"> minutes</td>
	</tr>
	<tr>
		<td><?=$language->show('M_DIRECTOR')?>: </td>
		<td colspan="2"><input type="text" name="director" value="<?=$obj->getDirector()?>" size="25"></td>
	</tr>
	<tr>
		<td><?=$language->show('M_COUNTRY')?>:</td>
		<td colspan="2"><input type="text" name="Country" value="<?=VCDUtils::split($obj->getCountry(), ", ");?>" size="40"></td>
	</tr>
	<tr>
		<td nowrap>IMDB <?=$language->show('M_CATEGORY')?>: </td>
		<td colspan="2"><input type="text" name="categories" value="<?=VCDUtils::split($obj->getGenre(), ", ");?>" size="40"></td>
	</tr>
	<tr>
		<td colspan="3"><?=$language->show('M_PLOT')?>:<br/>
		<textarea cols="55" rows="5" name="plot"><?=stripslashes($obj->getPlot())?></textarea></td>
	</tr>
	<tr>
...
if(is_array($obj->getCast(false))) {
	foreach ($obj->getCast(false) as $actor) {
