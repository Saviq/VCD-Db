...
<?
	if (sizeof($movies) == 0) {
		print "<p>".language::translate('ADD_NOTITLES')."<br/></p>";
	} else {
?>


<p class="bold"><?=language::translate('ADD_LISTEDSTEP1')?></p>
...
<form action="./exec_form.php?action=addlisted" method="post" name="choiceForm">
	...
	<tr>
	<td>
	<h2><?=language::translate('ADD_INDB')?></h2>
	...
	<td><h2><?=language::translate('ADD_SELECTED')?></h2>
		<select multiple name="choiceBox" id="choiceBox" style="width:300px;" size="8" class="input"></select>
		<br/>
		<input type="submit" onClick="return checkListed(this.form)" value="<?=language::translate('X_PROCEED')?> &gt;&gt;"/>
	</td>
	</tr>
	</table>
	<br/>
	<p><?=language::translate('ADD_INFOLIST')?></p>
